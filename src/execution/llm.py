...
규칙:
- 3~6개의 Task로 나눕니다. 각 Task는 사용자가 "오늘 이걸 하면 된다"고 알 만큼 구체적으로.
- 순서대로 배열합니다(먼저 할 일이 앞).
- 공고 원문에서 실제로 요구하는 것(팀 구성, 제출물, 필요 서류, 면접, 코딩테스트 등)을 반영합니다.
- 공모전·지원·신청 글이면 마지막 Task는 제출/신청으로 둡니다.
- 할인·패션·학습 팁처럼 신청 대상이 아닌 일반 정보면, 원문에 있는 내용만 바탕으로 활용 목표를 정하고 마지막 Task는 '다음 행동 정하기'처럼 실제 선택으로 마무리합니다. 신청이나 제출을 지어내지 마세요.
