...
flowchart TD
    ...
    U -->|링크/스크린샷/텍스트 저장| B_EXT["B: extraction_agent.py<br/>SavedContext"]
    B_EXT --> B_NORM["B: normalization_agent.py<br/>NormalizationResult<br/>(8종 자격조건 + raw_quote)"]
    A_PROFILE --> C_ELIG
    B_NORM --> C_ELIG["C: eligibility_agent.py<br/>충족/불충족 + 근거"]
    ...
    A_SUP["A: supervisor.py<br/>전체 오케스트레이션"] -.조율.-> B_EXT
    A_SUP -.조율.-> C_ELIG
    A_SUP -.조율.-> D_PLAN
...
- **SavedContext** (B 출력 #1): 사용자가 저장한 원본 → 정제된 텍스트. `status: ok/partial/failed` 필수 (R3).
- **NormalizationResult** (B 출력 #2, C의 입력): 자격조건 8종 리스트, 조건마다 `raw_quote` + `span` 필수 (R2, 가장 중요). 모르면 `operator="unknown"` (R4).
